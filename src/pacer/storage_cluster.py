"""Redis Cluster storage backend for distributed rate limiting.

This module is optional and only needed for Redis Cluster deployments.
For most applications, use SimpleRedisStorage from storage_simple.py instead.
"""
...
class RedisClusterStorage:
    """Redis Cluster storage backend for rate limiting with GCRA.

    Use this only if you need Redis Cluster support. For single Redis instances
    or Redis Sentinel setups, use SimpleRedisStorage instead.

    Note: Redis Cluster adds complexity and requires proper key design with
    hash tags to ensure related keys are on the same node.
    """
    ...
    async def connect(self) -> None:
        """Establish connection to Redis Cluster and load scripts."""
        try:
    ...
    async def _load_script(self) -> None:
        """Load GCRA Lua script into Redis Cluster nodes."""
        if not self._client:
            raise RuntimeError("Redis Cluster client not connected")
        ...
        # Note: In cluster mode, script needs to be loaded on all nodes
    ...
    async def check_rate_limit(
        ...
        Args:
            key: Redis key for the rate limit (must include hash tag for cluster)
        ...
        if not self._connected or not self._client:
            raise RuntimeError("Redis Cluster storage not connected")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    logger.warning(f"Redis Cluster operation failed (attempt {attempt + 1}), retrying: {e}")
                    await asyncio.sleep(0.01 * (2 ** attempt))  # Exponential backoff
                else:
                    logger.error(f"Redis Cluster operation failed after {self.max_retries + 1} attempts")
                    raise
            except ResponseError as e:
                logger.error(f"Redis Cluster script error: {e}")
                raise
    ...
    async def is_healthy(self) -> bool:
        """Check if Redis Cluster connection is healthy."""
        if not self._connected or not self._client:
            return False
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        try:
            # Note: info() might behave differently in cluster mode
            ...
            return {"connected": False, "cluster_mode": True}
